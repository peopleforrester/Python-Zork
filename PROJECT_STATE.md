...
- Tests: 693/693 via `uv run pytest`; 39 puzzles; ruff clean; mypy clean across `computerquest` and `scripts/`. Frontend: 61 vitest + 3 Playwright e2e green.
...
- 2026-08-03T00:00:00Z 2.3 CLI surface tested (game.py 76% -> 96%); move() stopped duplicating player.look()'s readout, pinned byte-exact by golden_move_output.json; PRD prose cleaned, zero em-dashes repo-wide; 693 tests
- 2026-08-03T00:00:00Z 2.3 full live playthrough at 39 puzzles: 39/39 solved, 25/25 knowledge, 5/5 quarantined, victory, turn 81, map header 39/39; first such run since the content grew from 29. Filed issue #6 (victory ends the run with no warning, stranding unsolved puzzles)
- 2026-08-04T00:00:00Z ops: custom domain https://zork.michaelrishiforrester.com added (Railway CNAME + _railway-verify TXT on Namecheap). Zone re-read immediately before the write caught two records added from outside the session; building from the 8-minute-old backup would have deleted codequest's domain. All 16 pre-existing records preserved, verified by diff.
- 2026-08-04T00:00:00Z 2.3 showcase readiness verified on production: 20 concurrent sessions isolated (7s wall), mobile tap-and-type works under emulation, 35-minute idle session survives, save-namespace collision fixed (per-browser key, hashed server-side) and isolation verified live; 707 pytest + 69 vitest
